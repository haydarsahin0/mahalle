...
import {features,ringFor,coordinates,parcel} from './land.js';
...
export function createMap(container,boundary,callbacks){
 ...
 function update(){if(!ready||!state)return;let fc=EMPTY,wide=false;
  if(map.getZoom()>=PARCEL_ZOOM){const b=map.getBounds();
   const result=features([b.getWest(),b.getSouth(),b.getEast(),b.getNorth()],state.week,state.holdings,900,3600,state.userId);
   if(result)fc=result;else wide=true;}
  map.getSource('parcels').setData(fc);
  homeLayer?.update(fc.features.map(f=>parcel(f.properties.id,state.week,state.holdings)));
 ...
 map.addSource('parcels',{type:'geojson',data:EMPTY});map.addLayer({id:'parcel-fill',type:'fill',source:'parcels',paint:{'fill-color':['case',['==',['get','owned'],1],'#6f9d72',['get','color']],'fill-opacity':['case',['==',['get','owned'],1],.52,['==',['get','arsa'],1],.3,.2]}});map.addLayer({id:'parcel-lines',type:'line',source:'parcels',paint:{'line-color':['case',['==',['get','owned'],1],'#3f6947',['get','color']],'line-width':['interpolate',['linear'],['zoom'],14,.7,17,1.6],'line-opacity':.95}});map.addLayer({id:'parcel-buildings',type:'fill-extrusion',source:'parcels',filter:['>', ['get','height'],0],paint:{'fill-extrusion-color':['case',['==',['get','owned'],1],'#6f9d72',['get','color']],'fill-extrusion-height':['get','height'],'fill-extrusion-opacity':.8}});
 ...
 map.on('click','parcel-fill',e=>{if(showGrid&&e.features?.length)callbacks.select(e.features[0].properties.id);});map.on('mouseenter','parcel-fill',()=>map.getCanvas().style.cursor='pointer');map.on('mouseleave','parcel-fill',()=>map.getCanvas().style.cursor='');ready=true;setMode(mode);update();if(selected)select(selected);callbacks.ready();import('./home-map.js').then(({createHomeLayer})=>{homeLayer=createHomeLayer();map.addLayer(homeLayer);homeLayer.setVisible(showGrid);update();}).catch(()=>callbacks.error('3D konutlar yüklenemedi. Sayfayı yenileyebilirsin.'));});
