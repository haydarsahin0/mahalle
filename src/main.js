...
import {online,currentUser,onAuthChange,recoverPayments,signInWithGoogle,logout,profile,holdingsIn,myHoldings,listedHoldings,recentParcels,act,startCheckout,startCustomCheckout,startParcelCheckout,confirmCheckout,reconcilePayments,startConnectOnboarding,claimWelcomeGift} from './api.js';
...
let user=null,lastReconcileAt=0,account_={name:'',welcomeGiftClaimed:true,welcomeGiftParcelId:null,stripeOnboardingComplete:false,firstHomeClaimed:true},map,selected=null,tab='explore',filter='all',visible=[],places=[],recent=[],busy=false,available=false,lastBox='',giftPromptedFor=null;
...
<aside><div id="side"></div><footer><span>Bir yer seç. Bir gelecek hayal et.</span><div class="footer-actions"><a href="./legal/" class="legal-link">Yasal bilgiler</a><button id="help" aria-label="Oyun hakkında">${icon('help')}</button></div></footer></aside></main><div id="toast" role="status" aria-live="polite"></div><dialog id="modal"></dialog>`;
function mine(p){return !!user&&p.owner===user.id;}
$('.brand').insertAdjacentHTML('afterend',`<a class="header-contact" href="mailto:patron@dijitalarsam.com" aria-label="İletişim: patron@dijitalarsam.com adresine e-posta gönder"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="3"/><path d="m4 7 8 6 8-6"/></svg><span><small>BİZE ULAŞ</small>patron@dijitalarsam.com</span></a>`);
...
async function syncAccount(next){
 ...
 if(!user){account_={name:'',welcomeGiftClaimed:true,welcomeGiftParcelId:null,stripeOnboardingComplete:false,firstHomeClaimed:true};giftPromptedFor=null;state.balance=0;draw();return;}
 try{const me=await profile(user.id);account_={name:me?.name||'Komşu',welcomeGiftClaimed:!!me?.welcome_gift_claimed,welcomeGiftParcelId:me?.welcome_gift_parcel_id||null,stripeOnboardingComplete:!!me?.stripe_onboarding_complete,firstHomeClaimed:!!me?.first_home_claimed};state.balance=Number(me?.balance||0);
  remember(await myHoldings(user.id));
  // Webhook kaçırılmış olsa bile, kullanıcının ödenmiş oturumlarını girişte bir kez uzlaştır.
  if(Date.now()-lastReconcileAt>30000){lastReconcileAt=Date.now();try{const reconciliation=await reconcilePayments();if(reconciliation?.balance!==null&&reconciliation?.balance!==undefined)state.balance=Number(reconciliation.balance);}catch(e){console.warn('payment reconciliation',e.message);}}
...
function detail(){const p=get(selected),zone=ZONES[p.zone],r=rumor(p,state.week),owned=mine(p),buyable=!p.owner||p.listing;
 ...
 const sameDay=p.lastCareAt&&new Date(p.lastCareAt).toISOString().slice(0,10)===new Date().toISOString().slice(0,10);
 const placed=Array.isArray(p.decorations)?p.decorations:[];
 const decorHtml=Object.entries(DECORATIONS).map(([key,item])=>{const has=placed.includes(key);return `<button data-decorate="${key}" ${has?'disabled':''}><span>${item.icon}</span><b>${has?'Arsanda var':item.name}</b><small>${has?'✓ kalıcı':fmt(item.cost)+' ◈'}</small></button>`;}).join('');
 let activity='';
 if(owned){
  if(p.buildBlockedUntil&&new Date(p.buildBlockedUntil)>new Date())activity+=`<div class="home-risk"><strong>İmarsız konutun yıkıldı.</strong><p>Bu parselde yeni yapı için bekleme bitişi: ${new Date(p.buildBlockedUntil).toLocaleString('tr-TR')}. Arsan sende kalır; hobi bahçende oynayabilirsin.</p></div>`;
  if(!p.building)activity+=`<div class="garden-invite"><h3>🏡 ${account_.firstHomeClaimed?'Yeni bir konut inşa et':'İlk konutun bizden!'}</h3><p>24 model · Level 1–5 · ${account_.firstHomeClaimed?'60 jeton':'Hesap başına bir kez ücretsiz'}${p.zone==='field'?' · İmarsız parselde %25 yıkım ihtimali':''}</p><button class="primary" id="choose-home" ${p.listing||(p.buildBlockedUntil&&new Date(p.buildBlockedUntil)>new Date())?'disabled':''}>Konut modellerini seç</button>${p.listing?'<p>İnşa etmek için önce ilanını kaldır.</p>':''}</div>`;
  if(p.building==='home')activity+=`<div class="plot-life"><h3>🏡 Level ${p.level} / 5 · ${p.homeFloors||1} kat</h3><p>Konut katkısı: +${p.homeValue} jeton oyun değeri.</p><button class="primary" id="home-upgrade" ${p.level>=5||p.listing?'disabled':''}>${p.level>=5?'En yüksek seviye':'Level '+(p.level+1)+' · '+upgradePrice(p)+' jeton'}</button></div>`;
  activity+=`<div class="garden-invite"><span>🌻</span><h3>Arsanda biraz vakit geçir.</h3><p>Otları temizle, ücretsiz tohum ek, sula ve hasat et. Bahçeni geliştirdikçe yeni çiçekler açılır.</p><button class="primary" id="open-garden">Hobi bahçemde oyna · Ücretsiz</button></div>`;
  activity+=`<div class="plot-life"><div class="life-head"><div><h3>✨ Arsanda bugün</h3><p>Her gün uğra, küçük bir ödül kazan.</p></div><span class="streak">${p.careStreak||0} gün</span></div><button class="free-action" id="care-action" ${sameDay?'disabled':''}>${sameDay?'✓ Bugünkü bakım tamamlandı':'🫶 Ücretsiz bakım yap · +1 jeton'}</button></div>`;
  if(p.building==='farm') activity+=`<div class="farm-cycle"><h3>🌱 Günlük üretim</h3>${p.crop?`<p><strong>${cropNames[p.crop]||p.crop}</strong> ${ready?'hasada hazır!':'büyüyor.'}</p><small>${ready?'Şimdi hasat edip küçük bir jeton ödülü kazanabilirsin.':'Her ürün 24 saatte olgunlaşır. Yarın tekrar gel.'}</small><button class="primary" id="${ready?'harvest':'farm-wait'}" ${ready?'':'disabled'}>${ready?'Hasat et · + jeton':'Olgunlaşması bekleniyor'}</button>`:`<p>Tarlana bir ürün seç, yarın hasat edip jeton kazan.</p><div class="crop-grid">${[['wheat','🌾','Buğday'],['olive','🫒','Zeytin'],['lavender','💜','Lavanta'],['greenhouse','🥬','Sera']].map(([k,i,n])=>`<button data-crop="${k}"><span>${i}</span><b>${n}</b><small>Ekim</small></button>`).join('')}</div>`}</div>`;
  else if(p.building) activity+=`<div class="rent-card"><h3>🏠 Kira geliri</h3><p>Bu yapıdan her 24 saatte küçük bir kira geliri toplayabilirsin.</p><button class="primary" id="collect-rent">Kira geliri topla · +${p.building==='home'?2:4} jeton</button></div>`;
  activity+=`<div class="decor-shop"><div class="life-head"><div><h3>🌼 Arsana renk kat</h3><p>Bir kez al, arsanda kalıcı olarak görünsün.</p></div><span class="decor-count">${placed.length}/6</span></div><div class="decor-grid">${decorHtml}</div></div>`;
 }
 ...
 $('#choose-home')?.addEventListener('click',async()=>{modal('<div id="home-picker">Modeller hazırlanıyor…</div>');const root=$('#home-picker');try{const {mountHomePicker}=await import('./home-picker.js');if(root.isConnected&&$('#modal').open)mountHomePicker(root,{free:!account_.firstHomeClaimed,cost:account_.firstHomeClaimed?60:0,risky:p.zone==='field',balance:state.balance,onBuild:data=>run('build',data)});}catch{root.textContent='Modeller yüklenemedi. Lütfen yeniden dene.';}});
 $('#home-upgrade')?.addEventListener('click',()=>confirmAction('upgrade',upgradePrice(p)));
 ...
 $('#open-garden')?.addEventListener('click',async()=>{modal('<div id="hobby-garden">Bahçen hazırlanıyor…</div>');const root=$('#hobby-garden');try{const {mountGarden}=await import('./garden.js');if(root.isConnected&&$('#modal').open)await mountGarden(root,user.id,p.id);}catch{if(root.isConnected)root.textContent='Bahçe açılamadı. Lütfen yeniden dene.';}});
 $('#care-action')?.addEventListener('click',()=>run('care'));
 document.querySelectorAll('[data-decorate]').forEach(b=>b.onclick=()=>run('decorate',{item:b.dataset.decorate}));
...
async function run(action,data={}){
 ...
 try{
  ...
  if(typeof result.firstHomeClaimed==='boolean')account_.firstHomeClaimed=result.firstHomeClaimed;
  if(Number.isFinite(result.balance))state.balance=Number(result.balance);
  if(result.parcel)state.holdings[result.parcel.id]={owner:result.parcel.owner_id,building:result.parcel.building||null,level:result.parcel.level||0,listing:result.parcel.listing||null,crop:result.parcel.crop||null,cropReadyAt:result.parcel.crop_ready_at||null,rentLastCollected:result.parcel.rent_last_collected||null,rentPrice:result.parcel.rent_price||null,decorations:Array.isArray(result.parcel.decorations)?result.parcel.decorations:[],lastCareAt:result.parcel.last_care_at||null,careStreak:Number(result.parcel.care_streak||0),homeModel:result.parcel.home_model||0,homeFloors:result.parcel.home_floors||1,buildBlockedUntil:result.parcel.build_blocked_until||null,homeUnpermitted:!!result.parcel.home_unpermitted};
  map?.render(state);draw();renderRecent();loadRecentParcels();$('#modal').close();
  if(result.demolished){toast('Oyun denetiminde konutun yıkıldı. Bu parselde 4 gün yeni yapı kuramazsın; ilk konut hakkı kullanılmış sayıldı.');return;}
  toast({buy:'Bu dijital parsel artık senin. 🌱',build:'Arsana yeni bir hayat geldi!',upgrade:'Yapının seviyesi yükseldi.',list:'İlanın yayınlandı.',unlist:'İlan kaldırıldı.',plant:'Ürün ekildi. Yarın hasat edebilirsin. 🌱',harvest:`Hasat tamamlandı. +${result.reward||0} jeton kazandın!`,rent:`Kira toplandı. +${result.reward||0} jeton kazandın!`,care:`Bakım tamamlandı. +${result.reward||0} jeton kazandın!`,decorate:'Arsana yeni bir öğe eklendi. ✨'}[action]);
