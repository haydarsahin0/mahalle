...
- A bundled Natural Earth Turkey polygon excludes foreign territory and most sea parcels; a parcel is kept when its reference point or any corner is on land, so coastal parcels may overhang the shoreline rather than leaving the land beside them empty. This generalized boundary is not cadastral and does not exclude every small lake, road or existing building.
...
- Parcels are not drawn on mapped lakes, and parcels outside the national boundary are dropped.
